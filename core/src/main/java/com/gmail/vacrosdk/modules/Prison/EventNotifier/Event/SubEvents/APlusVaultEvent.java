...
public class APlusVaultEvent extends NotificationEvent {

  public APlusVaultEvent(String catchMessage, FreakyvilleAddon addon) {
    super(catchMessage, addon);
  }

  @Override
  public void runSubEvent(ChatReceiveEvent event) {
    if(addon.configuration().getaPlusVaultEvent().get()) {
      runSound();
      Utils.showTitle("§6§lA+ Bank");
    }
  }
}
