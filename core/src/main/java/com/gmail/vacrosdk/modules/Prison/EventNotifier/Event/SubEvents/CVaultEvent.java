...
public class CVaultEvent extends NotificationEvent {

  public CVaultEvent(String catchMessage, FreakyvilleAddon addon) {
    super(catchMessage, addon);
  }

  @Override
  public void runSubEvent(ChatReceiveEvent event) {
    if(addon.configuration().getCVaultEvent().get()) {
      runSound();
      Utils.showTitle("§c§lC Vagtvault");
    }
  }
}
