...
public class BPlusVaultEvent extends NotificationEvent {

  public BPlusVaultEvent(String catchMessage, FreakyvilleAddon addon) {
    super(catchMessage, addon);
  }

  @Override
  public void runSubEvent(ChatReceiveEvent event) {
    if(addon.configuration().getBPlusVaultEvent().get()) {
      runSound();
      Utils.showTitle("§e§lB+ Bank");
    }
  }
}
