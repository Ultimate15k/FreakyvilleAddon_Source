...
public class BVaultEvent extends NotificationEvent {

  public BVaultEvent(String catchMessage, FreakyvilleAddon addon) {
    super(catchMessage, addon);
  }

  @Override
  public void runSubEvent(ChatReceiveEvent event) {
    if(addon.configuration().getBVaultEvent().get()) {
      runSound();
      Utils.showTitle("§b§lB Vagtvault");
    }
  }
}
